...
with open('./status/now.json', 'r') as now:
    start = json.load(now)
...
#field 이름
name =["pfCode","yr","period","periodDesc","aggrLevel","IsLeaf","rgCode","rgDesc","rtCode","rtTitle","rt3ISO","ptCode","ptTitle","pt3ISO","ptCode2","ptTitle2","pt3ISO2","cstCode","cstDesc","motCode","motDesc","cmdCode","cmdDescE","qtCode","qtDesc","qtAltCode","qtAltDesc","TradeQuantity","AltQuantity","NetWeight","GrossWeight","TradeValue","CIFValue","FOBValue","estCode"]
    ...
    for repoter in range(start['repoter'],39): #repoter 194, 39번
        ...
        for partner in range(start['partner'],40): #partner 197, 40번
            ...
            if k.status_code != 200:
                print('요청이 거절당했습니다 1시간 후에 다시 시도하세요.')
                print(f'현재시각{datetime.datetime.now()}')
                time.sleep(3600)

            # k = {'1':1, 'dataset':[{'11':11},{'22':22}]}
            content = k.json()
            extract = content['dataset']
            data_list = ([k.url,count,datetime.datetime.now()])
            ...
            with open('./status/now.json', 'w+') as file:
                file.write(json.dumps(s))
            excel_go = open(f'final.csv', 'a',encoding='utf-8',newline='')
            wr = csv.DictWriter(excel_go,fieldnames=name)
            wr.writeheader()
            wr.writerows(extract)
            excel_go.close()

            print(f'돌아간 횟수 :{count} api주소:{k.url}')
            # json 저장
            with open (f'./backup_10/url.json', 'a') as outfile:
                json.dump(data_list,outfile)
            ...
            #     print(f'{next_time.hour}에 시작할 수있습니다')
                ...
                # json 저장
                # with open (f'./backup_100/count_{count}.json', 'w') as outfile:
                #     json.dump(data_list,outfile)
                # print(f'second {count}번째 백업완료 ')

                # # csv 저장
                # excel_go = open(f'./csv_backup/test{repoter_count}.csv', 'w',newline='')
                # wr = csv.writer(excel_go)
                # wr.writerow(data_list[0])
                # for i in data_list:
                #     wr.writerow(i.values())
